...
export default function AdminDashboard() {
  ...
  const fetchArticles = async () => {
    try {
      const response = await fetch('https://stack-backend-qwon.onrender.com/api/blog');
  ...
  const deleteArticle = async (slug) => {
    ...
    try {
      await fetch(`https://stack-backend-qwon.onrender.com/api/blog/${slug}`, { method: 'DELETE' });
      fetchArticles();
    } catch (error) {
